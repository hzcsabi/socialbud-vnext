...
You must add the first admin manually (one-time), because only existing admins can use the Admins UI.
...
2. In the app, go to **Admin → Admins**. There you can add an admin by email (the user must already exist in Authentication → Users) and remove admin access. You cannot remove your own admin access.
